...
if 'full_db.db' not in script_directory:
    ...
    cursor.execute('CREATE TABLE IF NOT EXISTS news '
                   '(id INTEGER PRIMARY KEY AUTOINCREMENT, text TEXT, city TEXT, datetime TEXT)')
    cursor.execute('CREATE TABLE IF NOT EXISTS private_ad '
                   '(id INTEGER PRIMARY KEY AUTOINCREMENT, text TEXT, actual_until TEXT)')
    cursor.execute('CREATE TABLE IF NOT EXISTS personal_blog '
                   '(id INTEGER PRIMARY KEY AUTOINCREMENT, text TEXT, author TEXT)')
    connection.commit()
    connection.close()
...
class DatabaseHandler:
    ...
    def record_exists(self, table_name, fields, values):
        # SQL query to count records matching the specified conditions
        query = f'SELECT COUNT(*) FROM {table_name} WHERE {" AND ".join([f"{field} = ?" for field in fields])}'
        result = self.cursor.execute(query, tuple(values)).fetchone()
        # True if the count is greater than 0, indicating a record exists
        return result[0] > 0

    def insert_record(self, table_name, fields, values):
        if not self.record_exists(table_name, fields, values):
            # Query to handle insertion
            query = f'INSERT INTO "{table_name}" ({", ".join(fields)}) VALUES ({", ".join(["?"] * len(values))})'
            self.cursor.execute(query, tuple(values))
            self.connection.commit()
            print('INFO: Record inserted to DB successfully.')
        else:
            print('INFO: Duplicate record found. No record inserted to DB.')


class XMLPublisher:
    def __init__(self, newsfeed_path, xml_file_path=None):
    ...
    def publish_from_xml(self):
        try:
            if self.xml_file_path is None:
                # If xml_file_path is not provided, look for an XML file in the script's directory
                xml_files = [file for file in os.listdir(script_directory) if file.endswith('.xml')]
                if not xml_files:
                    print('ERROR: No XML files found in the script directory. Please provide a valid XML file path.')
                    return False
                elif len(xml_files) == 1:
                    self.xml_file_path = os.path.join(script_directory, xml_files[0])
                else:
                    print('Multiple XML files found in the script directory.')
                    return False
            ...
            for item in root.findall('item'):
                publication_type = item.find('publication_type').text.strip().capitalize()
                ...
                if publication_type and text:
                    ...
                    if publication_type == 'News':
                        ...
                        database_handler.insert_record(
                            'news',
                            ['text', 'city', 'datetime'],
                            [text, city, current_time]
                        )
                        ...
                        expiration_date_str = item.find('expiration_date').text.strip()
                        expiration_date = datetime.datetime.strptime(expiration_date_str, "%d/%m/%Y")
                        current_date = datetime.datetime.now()
                        days_left = (expiration_date - current_date).days
                        content_publisher.body = (f'Private ad -------------------\n{text}\nActual until: '
                                                  f'{expiration_date.strftime("%d/%m/%Y")}, {days_left} days left')
                        # Database insertion
                        database_handler = DatabaseHandler()
                        database_handler.insert_record(
                            'private_ad',
                            ['text', 'actual_until'],
                            [text, expiration_date.strftime("%d/%m/%Y")]
                        )
                        database_handler.close_connection()
            ...
            print(f'INFO. Successful. Publications added from XML. XML file: {self.xml_file_path} removed')
            return True

        except Exception as e:
            print(f'INFO. Failed to publish from XML. Error: {e}')
            return False
        ...
        try:
            if self.json_file_path is None:
                ...
                if not json_files:
                    ...
                    print('Multiple JSON files found in the script directory.')
                    return False
...
class ContentPublisher:
    ...
    def create_private_ad_body(self):
        ...
        database_handler.insert_record(
            'private_ad',
            ['text', 'actual_until'],
            [self.text, expiration_date.strftime("%d/%m/%Y")]
        )
        database_handler.close_connection()

        return (f'Private ad -------------------\n{self.text}\nActual until: {expiration_date.strftime("%d/%m/%Y")}, '
...
class InputHandler:
    ...
    def load_text_from_file():
        ...
        try:
            ...
            print(f'ERROR: Permission denied. Make sure you have the necessary permissions to read'
                  f' {file_path}')
        except Exception as e:
...
if __name__ == "__main__":
    ...
    user_choice = input('Choose an option (Create text / Load text / Load from JSON / Load from XML): ').strip().lower()
    ...
    elif user_choice == 'load from xml':
        xml_option = input('Choose an option (Provide XML path / Load from script directory): ').strip().lower()

        if xml_option == 'provide xml path':
            xml_file_path = input('Enter the XML file path: ').strip()
            xml_publisher = XMLPublisher(NEWSFEED_PATH, xml_file_path)
            xml_publisher.publish_from_xml()
        elif xml_option == 'load from script directory':
            xml_publisher = XMLPublisher(NEWSFEED_PATH)
            xml_publisher.publish_from_xml()
        else:
            print('Invalid option. Please choose either "Provide XML path" or "Load from script directory".')
